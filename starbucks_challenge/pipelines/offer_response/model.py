...
def train(clf, param_grid, X, y, scoring='f1', cv=5, verbose=0):
    ...
    grid = GridSearchCV(estimator=clf, param_grid=param_grid, scoring=scoring, cv=cv, verbose=verbose)
...
def calc_feature_importance(model, feature_cols):
    feat_imp = pd.DataFrame(model.feature_importances_,index=feature_cols.tolist(),columns=['feat_imp']).reset_index()

    feat_imp.rename(columns={'index': 'feature'}, inplace=True)
    feat_imp['imp_perc'] = np.round((feat_imp['feat_imp']/feat_imp['feat_imp'].sum())*100,2)
    feat_imp = feat_imp.sort_values(by=['imp_perc'], ascending=False).reset_index(drop=True)
...
def main():
    if len(sys.argv) == 3:
        ...
        X_train, X_test, y_train, y_test = orfeatures.create_training_data(offer_response_df, label_col='responded', test_size=0.3) 
