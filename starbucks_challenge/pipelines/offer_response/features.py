...
def check_label_balance(y, threshold=10):
    ...
    """
    value_counts_perc = round((y.squeeze().value_counts()/y.squeeze().count())*100,2)
    print(f'value stats:\n{value_counts_perc}\n')
    value_std = np.std(value_counts_perc)
    print(f'Values std: {value_std}, threshod: {threshold}.')
    if np.std(value_counts_perc) > threshold:
        print(f'WARN: std of values in dataset exceeded threshold, data may not be balanced.')
    else:
...
def create_training_data(offer_response_df: pd.DataFrame, label_col: str, test_size: float=0.3, cols_removal: list=COLUMNS_FOR_REMOVAL, features_to_scale: list=FEATURES_TO_SCALE):
    ...
    if np.sum(feature_df.isnull().sum()) > 0:
        raise Exception(f'Feature dataset has nulls in {np.sum(feature_df.isnull().sum()) } columns.')
